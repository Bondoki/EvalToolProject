...
public class Auswertung_Dendrimer_Linear_RCom2Com_Harmonic_WHAM {
	...
	public Auswertung_Dendrimer_Linear_RCom2Com_Harmonic_WHAM(String fdir, String fname, String dirDst)//, String skip, String current)
	{
		...
		*/
		...
		int NrOfAllHistogramms = 11;
		
		double lowerBoundary = 0.0-0.5;
		double higherBoundary = 64-0.5;
		int NrBins = 256;
		...
		for(int i = 0; i < NrOfAllHistogramms; i++)
		{
			...
			OffSetF[i]=0.0;
			EquLength[i]=0.0+2.0*i;
			SpringConstant[i]=1.0;//0.5;
		...
		*/
		...
		for(int nrHisto = 0; nrHisto < NrOfAllHistogramms; nrHisto++)
		{
			...
			LoadFile(fname+"_r0"+(int)EquLength[nrHisto]+"_k1.0.bfm",100,nrHisto);
			//LoadFile(fname+"_L"+(int)EquLength[nrHisto]+"_reduced.bfm",100,nrHisto);
		...
		*/
		...
		{
			// by dV~4*PI*dr
			//double testnorm = (HG_RCom2Com_Plain[nrHisto].GetNrInBinNormiert(i)/(4.0*Math.PI*HG_RCom2Com_Plain[nrHisto].GetIntervallThickness()));
	...
	//perform all calculation to achieve selfconsitency
		...
		double errorSumSquareFreeEnergy=0.0000001;
		...
		*/
		...
		for(int nrHisto = 0; nrHisto < NrOfAllHistogramms; nrHisto++)
		{
			...
			for(int i = 0; i < HG_RCom2Com_Plain[nrHisto].GetNrBins(); i++)
			{
				...
				// by dV=4*PI*dr
				//integral += 4.0*Math.PI*HG_RCom2Com_Plain[nrHisto].GetIntervallThickness()*HG_RCom2Com_StatistikTotal.GetAverageInBin(i)*Math.exp(- (0.5*SpringConstant[nrHisto]*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)-EquLength[nrHisto])*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)-EquLength[nrHisto])));
				
				// by dV=4*PI*r^2*dr
				//integral += 4.0*Math.PI*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)*HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i))*HG_RCom2Com_Plain[nrHisto].GetIntervallThickness()*HG_RCom2Com_StatistikTotal.GetAverageInBin(i)*Math.exp(- (0.5*SpringConstant[nrHisto]*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)-EquLength[nrHisto])*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)-EquLength[nrHisto])));
	...
	public static void main(String[] args) {
		...
		else new Auswertung_Dendrimer_Linear_RCom2Com_Harmonic_WHAM(args[0], args[1], args[2]);//,args[1],args[2]);
