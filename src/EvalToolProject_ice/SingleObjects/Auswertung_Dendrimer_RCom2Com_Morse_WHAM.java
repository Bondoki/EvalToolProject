...
public class Auswertung_Dendrimer_RCom2Com_Morse_WHAM {
	...
	public Auswertung_Dendrimer_RCom2Com_Morse_WHAM(String fdir, String fname, String dirDst, double morse_well_depth, double morse_parameter)//, String skip, String current)
	{
	...
	do{
		...
		for(int i = 0; i < HG_RCom2Com_StatistikTotal.GetNrBins(); i++)
		{
			...
			for(int nrHisto = 0; nrHisto < NrOfAllHistogramms; nrHisto++)
			{
				...
				//morse_well_depth, double morse_parameter
				 //morse V=morse_well_depth*( 1-exp(-morse_parameter*(r-r0) )^2
				 double morse_potential = morse_well_depth*(1.0-Math.exp(-morse_parameter*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)-EquLength[nrHisto]) ) )*(1.0-Math.exp(-morse_parameter*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)-EquLength[nrHisto]) ) );
				 
				 normTotal += HG_RCom2Com_Plain[nrHisto].GetNrOfCounts()*
						 Math.exp(- (morse_potential - OffSetF[nrHisto]) );
		...
		*/
		...
		for(int nrHisto = 0; nrHisto < NrOfAllHistogramms; nrHisto++)
		{
			...
			for(int i = 0; i < HG_RCom2Com_Plain[nrHisto].GetNrBins(); i++)
			{
				...
				integral += 4*Math.PI*(HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i)*HG_RCom2Com_Plain[nrHisto].GetRangeInBin(i))*HG_RCom2Com_Plain[nrHisto].GetIntervallThickness()*HG_RCom2Com_StatistikTotal.GetAverageInBin(i)*Math.exp(- morse_potential);
	...
	public static void main(String[] args) {
		...
		if(args.length != 5)
		{
			...
			System.out.println("USAGE: dirSrc/ FileLinearChains[.xo]  dirDst/ morse_well_depth morse_parameter");
		}
		...
		else new Auswertung_Dendrimer_RCom2Com_Morse_WHAM(args[0], args[1], args[2], Double.parseDouble(args[3]), Double.parseDouble(args[4]));//,args[1],args[2]);
