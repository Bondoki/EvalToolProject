...
public class Auswertung_Arc_NoReOrientierung {
	...
	public Auswertung_Arc_NoReOrientierung(String srcdir, String fname, String dstDir)
	{
		...
		minHistoCounterionsXY=-0.01;//25.01;
		maxHistoCounterionsXY=15.01;//25.01;
		int binHistoCounterionsXY = 20;//40
		
		minHistoCounterionsZ=-2.01;//40.01;
		maxHistoCounterionsZ=70.01;//40.01;
...
histo3D_Solvent.DateiSchliessen();
	...
	public static void main(String[] args) {
		...
		System.out.println("Calculation of Density of Counter-ions around HEP-Rod");
		new Auswertung_Arc_NoReOrientierung(args[0], args[1], args[2]);
	}
	...
	public void playSimulation(int frame)
	{
				...
				double ree_z_original  =importData.PolymerKoordinaten[90*(l-1)+ende+3][2]-importData.PolymerKoordinaten[90*(l-1)+start][2];
			           ...
			           ree_z_original /= 4.0;
			      */     
				double ree_x_original  =importData.PolymerKoordinaten[1][0]-importData.PolymerKoordinaten[240][0];
				double ree_y_original  =importData.PolymerKoordinaten[1][1]-importData.PolymerKoordinaten[240][1];
				double ree_z_original  =importData.PolymerKoordinaten[1][2]-importData.PolymerKoordinaten[240][2];
					      
			    double reeOrigin_x_original = importData.PolymerKoordinaten[240][0];
			    
			    double reeOrigin_y_original = importData.PolymerKoordinaten[240][1];
			    	   
			    double reeOrigin_z_original = importData.PolymerKoordinaten[240][2];
			    ...
			    */
				...
				for(int HEPCubes = 2; HEPCubes < 240; HEPCubes++)
				{
				    ...
				    double distance_New_xz = Math.sqrt(relDistance_X*relDistance_X + relDistance_Z*relDistance_Z);
				    double distance_yNew=Math.sqrt(relDistance_Y*relDistance_Y);
					...
					System.out.println("distancey: " + (distance_yNew) + "   newdistance: " + (distance_New_xz));
					...
					*/
					...
					if((distance_New_xz) < maxHistoCounterionsXY )
						if((distance_yNew) < maxHistoCounterionsZ && (distance_yNew) > minHistoCounterionsZ)
							HG2D_Rod.AddValue(distance_New_xz,distance_yNew);
						...
						if((distance_zNew) < maxHistoCounterionsZ && (distance_zNew) > minHistoCounterionsZ)
							{
								...
								double vecX2 = importData.PolymerKoordinaten[HEPCubes][0]-importData.PolymerKoordinaten[1][0];
								double vecY2 = importData.PolymerKoordinaten[HEPCubes][1]-importData.PolymerKoordinaten[1][1];
								double vecZ2 = importData.PolymerKoordinaten[HEPCubes][2]-importData.PolymerKoordinaten[1][2];
				...
				int offset = 240;//90*NrOfHeparin+(4*NrOfMonomersPerStarArm+1)*NrOfStars;
				
				//subtract 1 at the boundary because of overdominating end-positions
				/*for(int counterions = 2; counterions <= 239; counterions++)
				{
					//return ( ((x1-x2) < LATTICE_HALF_NEGATIV) ? (x1-x2+LATTICE) : ( ((x1-x2) > LATTICE_HALF) ? (x1-x2-LATTICE) : (x1-x2) ) );
