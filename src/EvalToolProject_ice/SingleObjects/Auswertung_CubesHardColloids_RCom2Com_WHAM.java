...
public class Auswertung_CubesHardColloids_RCom2Com_WHAM {
	...
	public Auswertung_CubesHardColloids_RCom2Com_WHAM(String fdir, String fname, String dirDst, double springconstant)//, String skip, String current)
	{
		...
		*/
		...
		for(int i = 0; i < NrOfAllHistogramms; i++)
		{
			...
			HG_RCom2Com_Plain[i] = new Histogramm(+6.125,30.0+0.125,60);
			HG_RCom2Com_Statistik[i] = new HistogrammStatistik(+6.125,30.0+0.125,60);
			OffSetF[i]=0.0;
			EquLength[i]=3.0+0.5*i;
			System.out.println("EquLength["+i+"]: " + EquLength[i]);
		}
		
		HG_RCom2Com_StatistikTotal = new HistogrammStatistik(+6.125,30.0+0.125,60);
		...
		*/
		...
		for(int nrHisto = 0; nrHisto < NrOfAllHistogramms; nrHisto++)
		{
			...
			LoadFile(fname+"R_"+EquLength[nrHisto]+"_c_0.1_C_0.2_HardColloids.bfm",1,nrHisto);
		}
		/*
	...
	public static void main(String[] args) {
		...
		else new Auswertung_CubesHardColloids_RCom2Com_WHAM(args[0], args[1], args[2], Double.parseDouble(args[3]));//,args[1],args[2]);
