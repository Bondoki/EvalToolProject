...
public class Auswertung_Dendrimer_Micelles_Orientierung_ArbitaryAxis {
	...
	public Auswertung_Dendrimer_Micelles_Orientierung_ArbitaryAxis(String srcdir, String fname, String dstDir)
	{
		...
		minHistoCounterionsZ=-64.01;//40.2;
		maxHistoCounterionsZ=64.01;//40.2;
		int binHistoCounterionsZ = 128;//25;
...
histo3D_Solvent.DateiSchliessen();
	...
	public static void main(String[] args) {
		...
		System.out.println("Calculation of Density of Counter-ions around HEP-Rod");
		new Auswertung_Dendrimer_Micelles_Orientierung_ArbitaryAxis(args[0], args[1], args[2]);
	}
	...
	public void playSimulation(int frame)
	{
				...
				*/
				...
				for (int l= 1; l <= 10; l++)
				{
					...
					// sample over unit sphere
					double xrnd =  0.0;
					double yrnd =  0.0;
					double zrnd =  0.0;
					
					do{
						xrnd =  Math.random()*Math.signum(Math.random()-0.5);
						yrnd =  Math.random()*Math.signum(Math.random()-0.5);
						zrnd =  Math.random()*Math.signum(Math.random()-0.5);
					}while(xrnd*xrnd+yrnd*yrnd+zrnd*zrnd > 1);
					
					//System.out.println(xrnd*xrnd+yrnd*yrnd+zrnd*zrnd +" < 1");
					
				double PlaneVektorX=xrnd;
				double PlaneVektorY=yrnd;
				double PlaneVektorZ=zrnd;
				
				double PlaneVektorLength = Math.sqrt(PlaneVektorX*PlaneVektorX+PlaneVektorY*PlaneVektorY+PlaneVektorZ*PlaneVektorZ);
				
				double PlaneVektorNormX= PlaneVektorX/PlaneVektorLength;
				double PlaneVektorNormY= PlaneVektorY/PlaneVektorLength;
				double PlaneVektorNormZ= PlaneVektorZ/PlaneVektorLength;
				...
				double ree_z_original = 0.0;// =importData.PolymerKoordinaten[240][2]-importData.PolymerKoordinaten[1][2];
				...
				if((importData.Attributes[HEPCubes] == 4) || (importData.Attributes[HEPCubes] == 5))
				{
					...
					//relDistance_X = co_Z-reeOrigin_z_original;//((co_X-reeOrigin_x_original) < -0.5*Lattice_x) ? (co_X-reeOrigin_x_original+Lattice_x) : ( ((co_X-reeOrigin_x_original) > 0.5*Lattice_x) ? (co_X-reeOrigin_x_original-Lattice_x) : (co_X-reeOrigin_x_original) ) ;
					//relDistance_Y = co_Y-reeOrigin_y_original;//((co_Y-reeOrigin_y_original) < -0.5*Lattice_y) ? (co_Y-reeOrigin_y_original+Lattice_y) : ( ((co_Y-reeOrigin_y_original) > 0.5*Lattice_y) ? (co_Y-reeOrigin_y_original-Lattice_y) : (co_Y-reeOrigin_y_original) ) ;
					//relDistance_Z = co_X-reeOrigin_x_original;//((co_Z-reeOrigin_z_original) < -0.5*Lattice_z) ? (co_Z-reeOrigin_z_original+Lattice_z) : ( ((co_Z-reeOrigin_z_original) > 0.5*Lattice_z) ? (co_Z-reeOrigin_z_original-Lattice_z) : (co_Z-reeOrigin_z_original) ) ;
					
					//projection on abitary axis
					
					double relDistanceProj_X = (relDistance_X*PlaneVektorNormX+relDistance_Y*PlaneVektorNormY+relDistance_Z*PlaneVektorNormZ)*PlaneVektorNormX;
					double relDistanceProj_Y = (relDistance_X*PlaneVektorNormX+relDistance_Y*PlaneVektorNormY+relDistance_Z*PlaneVektorNormZ)*PlaneVektorNormY;
					double relDistanceProj_Z = (relDistance_X*PlaneVektorNormX+relDistance_Y*PlaneVektorNormY+relDistance_Z*PlaneVektorNormZ)*PlaneVektorNormZ;
					
					double relDistanceProjOrtho_X = relDistance_X- relDistanceProj_X;
					double relDistanceProjOrtho_Y = relDistance_Y- relDistanceProj_Y;
					double relDistanceProjOrtho_Z = relDistance_Z- relDistanceProj_Z;
					
					
					double sign = (relDistance_X*PlaneVektorNormX+relDistance_Y*PlaneVektorNormY+relDistance_Z*PlaneVektorNormZ) > 0 ? 1 : -1;
					double distance_Z = sign*Math.sqrt(relDistanceProj_X*relDistanceProj_X+relDistanceProj_Y*relDistanceProj_Y+relDistanceProj_Z*relDistanceProj_Z);
					
					double distance_XY = Math.sqrt(relDistanceProjOrtho_X*relDistanceProjOrtho_X + relDistanceProjOrtho_Y*relDistanceProjOrtho_Y+ relDistanceProjOrtho_Z*relDistanceProjOrtho_Z);
				...
				//distribution of corona-cubes (linear chains)
				for(int HEPCubes = 1; HEPCubes <= importData.NrOfMonomers; HEPCubes++)
				if((importData.Attributes[HEPCubes] == 1) || (importData.Attributes[HEPCubes] == 2))
				{
					...
					if((distance_XY) < maxHistoCounterionsXY )
						if((distance_Z) < maxHistoCounterionsZ && (distance_Z) > minHistoCounterionsZ)
							HG2D_Counterions.AddValue(distance_XY,distance_Z);
					
					
					
				}
				
				//distribution of whole object-cubes (dendrimer+linear chains)
				for(int HEPCubes = 1; HEPCubes <= importData.NrOfMonomers; HEPCubes++)
				{
					...
					if((relDistance_X) < maxHistoCounterionsXY && (relDistance_X) > minHistoCounterionsXY )
						if((relDistance_Y) < maxHistoCounterionsXY && (relDistance_Y) > minHistoCounterionsXY)
							if((relDistance_Z) < maxHistoCounterionsZ && (relDistance_Z) > minHistoCounterionsZ)
								HG3D_Solvent.AddValue(relDistance_X, relDistance_Y, relDistance_Z);
				
					
					
					if((distance_XY) < maxHistoCounterionsXY )
						if((distance_Z) < maxHistoCounterionsZ && (distance_Z) > minHistoCounterionsZ)
							HG2D_Solvent.AddValue(distance_XY,distance_Z);
