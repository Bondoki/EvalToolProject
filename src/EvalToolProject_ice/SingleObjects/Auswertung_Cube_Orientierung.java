...
public class Auswertung_Cube_Orientierung {
	...
	public Auswertung_Cube_Orientierung(String srcdir, String fname, String dstDir)
	{
		...
		minHistoCounterionsXY=-25.01;//40.01;
		maxHistoCounterionsXY=25.01;//40.01;
		int binHistoCounterionsXY = 40;
		
		minHistoCounterionsZ=-12.01;//40.2;
		maxHistoCounterionsZ=12.01;//40.2;
		int binHistoCounterionsZ = 25;
		...
		histo2D_RodNormiert.DateiAnlegen(FileDirectoryDst+FileName+"_Histo2D_RodNormiert.dat", false);
		...
		histo2D_RodGnuplot.setzeZeile("set output \'"+FileDirectoryDst+FileName+"_Histo2D_RodNormiert.ps"+"\'");
		...
		histo2D_RodGnuplot.setzeZeile("set title \"density distribution of original NT (normalized)\" ");
		...
		histo2D_RodGnuplot.setzeZeile("splot \'"+FileDirectoryDst+FileName+"_Histo2D_RodNormiert.dat"+"\' matrix using (($1/"+binHistoCounterionsXY+")*"+maxHistoCounterionsXY+"):("+minHistoCounterionsZ+"+($2/"+binHistoCounterionsZ+")*"+(maxHistoCounterionsZ-minHistoCounterionsZ)+"):3 ");
...
histo3D_Solvent.DateiSchliessen();
	...
	public static void main(String[] args) {
		...
		System.out.println("Calculation of Density of Counter-ions around HEP-Rod");
		new Auswertung_Cube_Orientierung(args[0], args[1], args[2]);
	}
	...
	public void playSimulation(int frame)
	{
				...
				double ree_z_original  =importData.PolymerKoordinaten[90*(l-1)+ende+3][2]-importData.PolymerKoordinaten[90*(l-1)+start][2];
			           ...
			           ree_z_original /= 4.0;
			      */     
				double ree_x_original  =importData.PolymerKoordinaten[20][0]-importData.PolymerKoordinaten[20][0];
				double ree_y_original  =importData.PolymerKoordinaten[20][1]+1-importData.PolymerKoordinaten[20][1];
				double ree_z_original  =importData.PolymerKoordinaten[20][2]+10-importData.PolymerKoordinaten[20][2];
					      
			    double reeOrigin_x_original = importData.PolymerKoordinaten[20][0];
			    
			    double reeOrigin_y_original = importData.PolymerKoordinaten[20][1];
			    	   
			    double reeOrigin_z_original = importData.PolymerKoordinaten[20][2];
				...
				*/
				...
				for(int HEPCubes = 1; HEPCubes <= 23; HEPCubes++)
				{
				...
				int offset = 23;//90*NrOfHeparin+(4*NrOfMonomersPerStarArm+1)*NrOfStars;
				
				//subtract 1 at the boundary because of overdominating end-positions
				for(int counterions = 1; counterions <= 23; counterions++)
				{
					//return ( ((x1-x2) < LATTICE_HALF_NEGATIV) ? (x1-x2+LATTICE) : ( ((x1-x2) > LATTICE_HALF) ? (x1-x2-LATTICE) : (x1-x2) ) );
