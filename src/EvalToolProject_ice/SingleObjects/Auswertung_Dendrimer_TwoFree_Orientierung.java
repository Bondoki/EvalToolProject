...
public class Auswertung_Dendrimer_TwoFree_Orientierung {
	...
	public Auswertung_Dendrimer_TwoFree_Orientierung(String srcdir, String fname, String dstDir)
	{
		FileName =  fname;
		...
		minHistoCounterionsZ=-128.01;
		maxHistoCounterionsZ=128.01;
		int binHistoCounterionsZ = 256;
		...
		histo3D_Counterions.DateiAnlegen(FileDirectoryDst+FileName+"_Histo3D_TwoDendrimerCentered.vtk", false);
		histo_Distance_Counterions.DateiAnlegen(FileDirectoryDst+FileName+"_Histo_Distance_Counterions.dat", false);
		
		
		histo2D_Counterions.DateiAnlegen(FileDirectoryDst+FileName+"_Histo2D_TwoDendrimerCentered.dat", false);
		histo2D_CounterionsGnuplot.DateiAnlegen(FileDirectoryDst+FileName+"_Histo2D_TwoDendrimerCenteredGnuplot.gnu", false);
		
		histo2D_CounterionsGnuplot.setzeZeile("set term postscript enhanced color");
		histo2D_CounterionsGnuplot.setzeZeile("set output \'"+FileDirectoryDst+FileName+"_Histo2D_TwoDendrimerCentered.ps"+"\'");
		...
		histo2D_CounterionsGnuplot.setzeZeile("set title \"density distribution of Two Dendrimer (centered)\" ");
		histo2D_CounterionsGnuplot.setzeZeile("unset key");
		histo2D_CounterionsGnuplot.setzeZeile("set xrange[0:20]");
		histo2D_CounterionsGnuplot.setzeZeile("set yrange[-20:60]");
		...
		histo2D_CounterionsGnuplot.setzeZeile("splot \'"+FileDirectoryDst+FileName+"_Histo2D_TwoDendrimerCentered.dat"+"\' matrix using (($1/"+binHistoCounterionsXY+")*"+maxHistoCounterionsXY+"):(0.5*"+HG2D_Counterions.GetIntervallThicknessOfBins2()+"+"+minHistoCounterionsZ+"+($2/"+binHistoCounterionsZ+")*"+(maxHistoCounterionsZ-minHistoCounterionsZ)+"):3 ");
		histo2D_CounterionsGnuplot.setzeLeerzeile();
		histo2D_CounterionsGnuplot.DateiSchliessen();

		
		histo2D_Rod.DateiAnlegen(FileDirectoryDst+FileName+"_Histo2D_TwoDendrimer.dat", false);
		histo2D_RodGnuplot.DateiAnlegen(FileDirectoryDst+FileName+"_Histo2D_TwoDendrimerGnuplot.gnu", false);
		
		histo2D_RodGnuplot.setzeZeile("set term postscript enhanced color");
		histo2D_RodGnuplot.setzeZeile("set output \'"+FileDirectoryDst+FileName+"_Histo2D_TwoDendrimer.ps"+"\'");
		...
		histo2D_RodGnuplot.setzeZeile("set title \"density distribution of Two Dendrimer (aligned)\" ");
		histo2D_RodGnuplot.setzeZeile("unset key");
		histo2D_RodGnuplot.setzeZeile("set xrange[0:20]");
		histo2D_RodGnuplot.setzeZeile("set yrange[-20:60]");
		...
		histo2D_RodGnuplot.setzeZeile("splot \'"+FileDirectoryDst+FileName+"_Histo2D_TwoDendrimer.dat"+"\' matrix using (($1/"+binHistoCounterionsXY+")*"+maxHistoCounterionsXY+"):(0.5*"+HG2D_Rod.GetIntervallThicknessOfBins2()+"+"+minHistoCounterionsZ+"+($2/"+binHistoCounterionsZ+")*"+(maxHistoCounterionsZ-minHistoCounterionsZ)+"):3 ");
		...
		histo3D.setzeZeile("Histogram Two Dendrimer Aligned at COM first");
		...
		histo3D_Counterions.setzeZeile("Histogram Two Dendrimer Centered at direction vector");
	...
	public static void main(String[] args) {
		...
		System.out.println("Calculation of Density of Counter-ions around HEP-Rod");
		new Auswertung_Dendrimer_TwoFree_Orientierung(args[0], args[1], args[2]);
	}
	...
	public void playSimulation(int frame)
	{
				...
				*/
				...
				for(int HEPCubes = 1; HEPCubes <= importData.NrOfMonomers; HEPCubes++)
				{
					...
					int co_X = importData.PolymerKoordinaten[HEPCubes][0];
					int co_Y = importData.PolymerKoordinaten[HEPCubes][1];
					int co_Z = importData.PolymerKoordinaten[HEPCubes][2];
				...
				for(int counterions = 1; counterions <= importData.NrOfMonomers; counterions++)
				{
					...
					int co_X = importData.PolymerKoordinaten[counterions][0];
					int co_Y = importData.PolymerKoordinaten[counterions][1];
					int co_Z = importData.PolymerKoordinaten[counterions][2];
				   ...
				   // System.out.println("xNewTrafo: " + distance_xNew + "   YNewTrafo: " + distance_yNew + "   ZNewTrafo: " + distance_zNew);
					...
					if((distance_xNew) < maxHistoCounterionsXY && (distance_xNew) > minHistoCounterionsXY )
						if((distance_yNew) < maxHistoCounterionsXY && (distance_yNew) > minHistoCounterionsXY)
							...
							HG3D_Counterions.AddValue(distance_X, distance_Y, distance_Z);
					//sqrt(2*L²/4)
					if(distance_New_xy < 90.0)
						HG_Distance_Counterions.AddValue(distance_New_xy);
