...
public class Auswertung_HEPPEG_Rg2All_BMC {
	...
	int NrOfHeparinInBMC = 0;
	int NrOfStarsInBMC = 0;
	...
	Statistik Rg2_BMC_stat;
	
	int[] PEGBonds;
	
	
	BFMFileSaver ClusterSaver;
	BFMFileSaver ClusterSaverBare;
	...
	public Auswertung_HEPPEG_Rg2All_BMC(String fdir, String fname, String gamma, String dstDir, String Experiment)//, String skip, String current)
	{
		...
		ClusterSaverBare = new BFMFileSaver();
		ClusterSaverBare.DateiAnlegen(dstDir+"/Percolation_BMC_DistributionBare_HepPEGConnectedGel_"+FileName+".dat", false);
		ClusterSaverBare.setzeZeile("# crosslinks(all) <BMC Monomers/AllMonomers>");
		...
		ClusterSaverBare.DateiSchliessen();
		
		BFMFileSaver RGSaver = new BFMFileSaver();
		RGSaver.DateiAnlegen(dstDir+"/"+ FileName+"_Rg2_All_N.dat", true);
		//RGSaver.setzeZeile("# MSD for Stars");
		RGSaver.setzeZeile( NrOfHeparin + " " + NrOfStars + " "+NrOfHeparinInBMC + " " + NrOfStarsInBMC + " "+ (NrOfMonomersPerStarArm*4+1) + " " + Math.sqrt(Rg2_BMC_stat.ReturnM1())+ " " + Rg2_BMC_stat.ReturnM1()+" "+Rg2_BMC_stat.ReturnM2()+" "+( 2.0* Rg2_BMC_stat.ReturnSigma()/ Math.sqrt(1.0*Rg2_BMC_stat.ReturnN() ))+ " " + Rg2_BMC_stat.ReturnN());
	...
	public static void main(String[] args) {
		...
		new Auswertung_HEPPEG_Rg2All_BMC(args[0], args[1], args[2], args[3], args[4]);//,args[1],args[2]);
	}
	...
	public void playSimulation(int frame)
	{
				...
				for(int it = (82*NrOfHeparin+3* NrOfStars); it < importData.addedBondsInBeginning.size(); it++)
				{
					System.out.println("addedBondsInBeginning: " + it);
					resetArrays();
					
					long bondobj = importData.addedBondsInBeginning.get(it);
				...
				{
					System.out.println("addedBondsBetweenFrames: " + it);
					resetArrays();
				...
				if(frame >= (NrofFrames-300))
				{
					...
					Rg2_BMC_stat.AddValue(RadiusOfGyration2);
					
					 NrOfHeparinInBMC = NrOfHeparinInCluster;
					 NrOfStarsInBMC=NrOfStarsInCluster;
				...
				ClusterSaverBare.setzeZeile(counterAdditionalBonds/(4.0*NrOfStars) + " "+  (117*StarClusterSize[findMaximumArrayValueIndex()]+90*HeparinClusterSize[findMaximumArrayValueIndex()])/(117.0*NrOfStars+90.0*NrOfHeparin));
				
				System.out.println("size: " +importData.addedBondsBetweenFrames.size());
				importData.addedBondsBetweenFrames.clear();
				importData.addedBondsInBeginning.clear();
	}
	...
	private int findMaximumArrayValueIndex()
	{
		...
		for (int i=1; i <= NrOfStars; i++)
		{
			//if(ColorHeparin[i] > 0)
				...
				if((StarClusterSize[i]+HeparinClusterSize[i]) > maximum) {
		            maximum = (StarClusterSize[i]+HeparinClusterSize[i]);   // new maximum
		            maximumIndex = i;
		        }
