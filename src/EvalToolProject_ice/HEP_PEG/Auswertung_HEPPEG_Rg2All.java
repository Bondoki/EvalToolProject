...
public class Auswertung_HEPPEG_Rg2All {
	...
	public Auswertung_HEPPEG_Rg2All(String SrcDir, String SrcName, String DstDir)//, String skip, String current)
	{
		...
		skipFrames =  10;//Integer.parseInt(skip);
		...
		RGSaver.setzeZeile( NrOfHeparin + " " + NrOfStars + " "+ (NrOfMonomersPerStarArm*4+1) + " " + Math.sqrt(Rg2all_stat.ReturnM1())+ " " + Rg2all_stat.ReturnM1()+" "+Rg2all_stat.ReturnM2()+" "+( 2.0* Rg2all_stat.ReturnSigma()/ Math.sqrt(1.0*Rg2all_stat.ReturnN() ))+ " " + Rg2all_stat.ReturnN());
	...
	public static void main(String[] args) {
		...
		else new Auswertung_HEPPEG_Rg2All(args[0], args[1], args[2]);
	}
	...
	public void playSimulation(int frame)
	{
				...
				//if(importData.MCSTime > 10000000)
				...
				rg_z_stat.clear();
				 ...
				 int offset = 0;// 90*NrOfHeparin;
					...
					//  for (int j = i; j <= ( 4*NrOfMonomersPerStarArm + 1)*NrOfStars+NrOfCrosslinker; j++)
						  for (int i= 1; i <= NrOfHeparin*90+NrOfStars*117; i++)
							  for (int j = i; j <= NrOfHeparin*90+NrOfStars*117; j++)
					  {
					...
					Rg2 = Rg2_x + Rg2_y + Rg2_z;
					  ...
					  Rg2_x /= 1.0*(NrOfHeparin*90+NrOfStars*117)*(NrOfHeparin*90+NrOfStars*117);
					  Rg2_y /= 1.0*(NrOfHeparin*90+NrOfStars*117)*(NrOfHeparin*90+NrOfStars*117);
					  Rg2_z /= 1.0*(NrOfHeparin*90+NrOfStars*117)*(NrOfHeparin*90+NrOfStars*117);
					  Rg2 /= 1.0*(NrOfHeparin*90+NrOfStars*117)*(NrOfHeparin*90+NrOfStars*117);
