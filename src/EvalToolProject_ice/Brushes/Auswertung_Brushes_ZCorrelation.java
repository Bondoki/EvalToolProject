package EvalToolProject_ice.Brushes;
...
public class Auswertung_Brushes_ZCorrelation {
	...
	Statistik[] MeanHeigth_Time_Stat;
	...
	public Auswertung_Brushes_ZCorrelation(String fdir, String fname, String dirDst, int nrOfMonoPerChain, int nrOfChains, int _startAtFrame, int _diffFrame)//, String skip, String current)
	{
...
//		rN2_Z = new double[maxframe+1][nrOfChains]; //[time][idx=chainnumber]
		...
		MeanHeigth_Time_Stat = new Statistik[maxframe+1];
...
//		g4_Time_Stat = new Statistik[maxframe+1];
		...
		{
			MeanHeigth_Time_Stat[i] = new Statistik();
...
//			g4_Time_Stat[i] = new Statistik();
		...
		*/
		...
		CorrSaver.DateiAnlegen(dirDst+"/"+FileName+"_ZCorr_Time_df"+diffFrame+"_f0_"+(startAtFrame+1)+".dat", false);
		...
		for(int time=0; time < (maxframe-startAtFrame); time++)
		{
			if(ZCorr_Time_Stat_First[time].ReturnN() > 2)
				CorrSaver.setzeZeile((deltaT*time) + " " + ZCorr_Time_Stat_First[time].ReturnM1()+ " " + ZCorr_Time_Stat_Last[time].ReturnM1() + " "+ZCorr_Time_Stat_Last[time].ReturnN());
		}
		
		CorrSaver.DateiSchliessen();
		
		BFMFileSaver CorrSaverNormiert = new BFMFileSaver();
		CorrSaverNormiert.DateiAnlegen(dirDst+"/"+FileName+"_ZCorrNormiert_Time_df"+diffFrame+"_f0_"+(startAtFrame+1)+".dat", false);
		...
		{
			if(ZCorr_Time_Stat_First[time].ReturnN() > 2)
				CorrSaverNormiert.setzeZeile((deltaT*time) + " " + (ZCorr_Time_Stat_First[time].ReturnM1()-ZCorr_Stat_First.ReturnM1()*ZCorr_Stat_First.ReturnM1())/(ZCorr_Stat_First.ReturnM2()-ZCorr_Stat_First.ReturnM1()*ZCorr_Stat_First.ReturnM1())+ " " + (ZCorr_Time_Stat_Last[time].ReturnM1()-ZCorr_Stat_Last.ReturnM1()*ZCorr_Stat_Last.ReturnM1())/(ZCorr_Stat_Last.ReturnM2()-ZCorr_Stat_Last.ReturnM1()*ZCorr_Stat_Last.ReturnM1()) + " "+ZCorr_Time_Stat_Last[time].ReturnN());
		}
		
		CorrSaverNormiert.DateiSchliessen();
		
		BFMFileSaver MeanHeigthSaver = new BFMFileSaver();
		MeanHeigthSaver.DateiAnlegen(dirDst+"/"+FileName+"_MeanHeigth_Time_df"+diffFrame+"_f0_"+(startAtFrame+1)+".dat", false);
		MeanHeigthSaver.setzeZeile("# NrOfMonomersPerChain="+ NrOfMonoPerChain);
		MeanHeigthSaver.setzeZeile("# NrOfChains="+ NrOfChains);
		MeanHeigthSaver.setzeZeile("# NrDensity c="+((8.0*(MONOMERZAHL-1))/(Gitter_x*Gitter_y*Gitter_z)));
		MeanHeigthSaver.setzeZeile("# tBeginn = "+ (startAtFrame*deltaT) + "    startAtFrame(+1) = " + (startAtFrame+1));
		MeanHeigthSaver.setzeZeile("# dt = "+ (diffFrame*deltaT) + "    dframe = " + diffFrame);
		MeanHeigthSaver.setzeZeile("# <MeanHeigth> = 1/(M*N) sum_M Sum_N z_m,n");
		MeanHeigthSaver.setzeZeile("# t  <MeanHeigth> SampleSize");
		
		for(int time=0; time < maxframe; time++)
		{
			MeanHeigthSaver.setzeZeile((deltaT*time) + " " + MeanHeigth_Time_Stat[time].ReturnM1() + " "+MeanHeigth_Time_Stat[time].ReturnN());
		}
		
		MeanHeigthSaver.DateiSchliessen();
	...
	public static void main(String[] args) {
		...
		else new Auswertung_Brushes_ZCorrelation(args[0], args[1], args[2] , Integer.parseInt(args[3]) , Integer.parseInt(args[4]), Integer.parseInt(args[5]), Integer.parseInt(args[6]));//,args[1],args[2]);
	...
	public void playSimulation(int frame)
	{
				...
				for (int nrChains= 0; nrChains < NrOfChains; nrChains++)
				{
					...
					*/
					...
					MeanHeigth_Time_Stat[frame-1].AddValue(Rcom_z);
