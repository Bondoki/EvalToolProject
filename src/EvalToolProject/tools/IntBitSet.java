...
public class IntBitSet {
	...
	if (fromIndex < 0)
        ...
        if (length() <= fromIndex || fromIndex == toIndex)
            return new IntBitSet(0);

        // An optimization
        if (length() < toIndex)
            toIndex = length();

        IntBitSet result = new IntBitSet(toIndex - fromIndex);
     ...
     * @since     JDK1.2
     */
    public void andNot(IntBitSet set) {
        int unitsInCommon = Math.min(unitsInUse, set.unitsInUse);
     ...
     */
    public Object clone() {
	IntBitSet result = null;
	try {
	    result = (IntBitSet) super.clone();
