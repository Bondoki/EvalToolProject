...
public abstract class IntAbstractList extends IntAbstractCollection implements IntList {
    /**
     * Sole constructor.  (For invocation by subclass constructors, typically
     * implicit.)
     */
    protected IntAbstractList() {
    }
   ...
   /* public int lastIndexOf(Object o) {
	...
	public void remove() {
	    ...
	    try {
		IntAbstractList.this.remove(lastRet);
		if (lastRet < cursor)
		    cursor--;
		lastRet = -1;
	...
	public void set(int o) {
	    ...
	    try {
		IntAbstractList.this.set(lastRet, o);
		expectedModCount = modCount;
	    } catch(IndexOutOfBoundsException e) {
		throw new ConcurrentModificationException();
	    }
	...
	public void add(int o) {
	    ...
	    try {
		IntAbstractList.this.add(cursor++, o);
		lastRet = -1;
		expectedModCount = modCount;
	    } catch(IndexOutOfBoundsException e) {
		throw new ConcurrentModificationException();
...
class SubList extends IntAbstractList {
    private IntAbstractList l;
    private int offset;
    private int size;
    private int expectedModCount;

    SubList(IntAbstractList list, int fromIndex, int toIndex) {
        if (fromIndex < 0)
    ...
    public boolean remove(int index) {
        rangeCheck(index);
        checkForComodification();
        //int result = l.remove(index+offset);
        expectedModCount = l.modCount;
        size--;
        modCount++;
        return true;
...
class RandomAccessSubList extends SubList implements RandomAccess {
    RandomAccessSubList(IntAbstractList list, int fromIndex, int toIndex) {
